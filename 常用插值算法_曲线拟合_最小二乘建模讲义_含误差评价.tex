...
\newpage
\section{模型评价：误差评价、拟合优度与残差诊断}

前面几章解决的是“怎样构造插值函数或拟合函数”，本章解决的是建模论文和考试中常问的另一个问题：\textbf{模型算出来以后，怎样说明它好不好？} 对曲线拟合、最小二乘优化、函数逼近和插值问题，都应该给出误差评价或拟合优度说明。

\begin{keybox}{评价的基本思路}
设观测值为 $y_i$，模型预测值为 $\hat y_i=f(x_i)$，残差为
\[
 e_i = y_i-\hat y_i.
\]
...
\begin{longtable}{p{2.7cm}p{5.4cm}p{5.9cm}}
\toprule
指标 & 公式 & 含义与适用场景 \\
\midrule
残差 Residual & $e_i=y_i-\hat y_i$ & 单个点的预测偏差。残差图是判断模型是否有系统偏差的基础。 \\
\midrule
SSE/RSS & $\mathrm{SSE}=\sum_{i=1}^n (y_i-\hat y_i)^2$ & 平方残差和，是最小二乘法直接最小化的目标。值越小越好，但会随样本量和量纲改变。 \\
\midrule
MSE & $\mathrm{MSE}=\frac{1}{n}\sum_{i=1}^n (y_i-\hat y_i)^2$ & 均方误差。对大误差更敏感，适合不希望出现较大偏差的场景。 \\
\midrule
RMSE & $\mathrm{RMSE}=\sqrt{\mathrm{MSE}}$ & 均方根误差，量纲与 $y$ 相同，最适合写进建模结果解释。 \\
\midrule
MAE & $\mathrm{MAE}=\frac{1}{n}\sum_{i=1}^n |y_i-\hat y_i|$ & 平均绝对误差。比 MSE/RMSE 对异常值更稳健，解释为“平均差多少”。 \\
\midrule
Max Error & $\max_i |y_i-\hat y_i|$ & 最大绝对误差。适合工程容差、数值查表、插值精度上界检查。 \\
\midrule
MAPE & $\mathrm{MAPE}=\frac{100\%}{n}\sum_{i=1}^n\left|\frac{y_i-\hat y_i}{y_i}\right|$ & 平均百分比误差。便于跨量纲比较，但当 $y_i$ 接近 0 时会失真或不可用。 \\
\bottomrule
\end{longtable}

\begin{warnbox}{不要只看一个指标}
RMSE 对大误差敏感，MAE 更稳健，MAPE 便于解释百分比但怕真实值接近 0。实际建模中常用组合是：\textbf{RMSE + MAE + $R^2$ + 残差图}。如果是插值查表问题，可再补充最大误差 $\max |e_i|$。
\end{warnbox}

\subsection{拟合优度：$R^2$ 与调整 $R^2$}
...
\]
其中 $\mathrm{SST}$ 表示只用均值 $\bar y$ 预测时的总波动，$\mathrm{SSE}$ 表示模型没有解释掉的误差。因此，$R^2$ 可理解为模型解释数据波动的比例。

\begin{keybox}{$R^2$ 的解释}
\begin{itemize}
  \item $R^2$ 越接近 1，说明拟合点上的误差相对总波动越小。
  \item $R^2=0$ 表示模型效果和“全部预测为均值”差不多。
  \item $R^2<0$ 说明模型甚至不如直接用均值预测，这在测试集或不合适模型中可能出现。
  \item 只增加模型复杂度通常会让训练集 $R^2$ 不下降，所以不能只用训练集 $R^2$ 判断模型好坏。
\end{itemize}
\end{keybox}
...
\begin{warnbox}{$R^2$ 的常见误区}
$R^2$ 高不代表模型一定适合外推，也不代表因果关系成立。高阶多项式可能在训练点上取得很高的 $R^2$，但区间外预测极差。因此建模报告中应同时给出验证误差、残差图和模型复杂度说明。
\end{warnbox}

\subsection{插值问题中的误差评价}

插值函数在给定节点上满足 $s(x_i)=y_i$，所以\textbf{节点训练误差通常为 0}。因此，插值不能只看节点误差，而应看以下内容：

\begin{enumerate}[label=\arabic*)]
  \item \textbf{已知真实函数时}：在更密的测试点 $t_j$ 上计算 $|f(t_j)-s(t_j)|$、RMSE 和最大误差。
  \item \textbf{未知真实函数时}：留出一部分观测点不参与插值，用这些点检验误差。
  \item \textbf{工程查表时}：重点报告最大绝对误差、最大相对误差和端点附近误差。
  \item \textbf{二维插值时}：可用误差热力图或等高线图展示误差在平面上的分布。
\end{enumerate}
...
\subsection{残差诊断：图比单个数值更重要}

残差图常用来判断模型形式是否合理。拟合完成后建议画出：
\begin{enumerate}[label=\arabic*)]
  \item \textbf{残差-自变量图}：若残差随机分布在 0 附近，模型较合适；若出现弯曲趋势，说明模型形式可能缺项。
  \item \textbf{残差-拟合值图}：若出现漏斗形，说明方差可能不恒定，可考虑加权最小二乘或变量变换。
  \item \textbf{残差直方图/QQ 图}：用于粗略检查误差是否接近正态，主要服务于统计推断。
  \item \textbf{异常点检查}：单个残差过大可能来自测量错误、异常样本或模型局部失效。
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/error_metrics_residuals.png}
\caption{误差评价示例：左图为数据与拟合曲线，右图为残差图。若残差在 0 附近随机波动，通常比出现明显趋势更理想。}
\end{figure}

\subsection{训练误差、测试误差与交叉验证}

建模时常把数据分为训练集和测试集。训练集用于确定模型参数，测试集用于估计泛化能力。一般来说：
\[
\text{训练误差很小，测试误差也小} \Rightarrow \text{模型较可靠};
\]
\[
\text{训练误差很小，测试误差很大} \Rightarrow \text{可能过拟合};
\]
\[
\text{训练误差和测试误差都大} \Rightarrow \text{可能欠拟合或模型形式错误}.
\]
...
\subsection{Python 评价指标代码}

下面代码可直接接在前面任意拟合或插值模型后使用，只要已有真实值 \verb|y_true| 和预测值 \verb|y_pred| 即可。

\begin{lstlisting}
import numpy as np
from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score, max_error

def regression_report(y_true, y_pred, p=None):
    ...
    # adjusted R^2: p = number of explanatory variables or effective parameters
    if p is not None and n > p + 1:
        out["Adjusted_R2"] = 1 - (1 - r2) * (n - 1) / (n - p - 1)
    return out

# example
# y_pred = model(x_test)
# print(regression_report(y_test, y_pred, p=3))
\end{lstlisting}
...
\subsection{建模论文中的规范写法}

\begin{stepbox}{评价指标段落模板}
设真实观测值为 $y_i$，模型预测值为 $\hat y_i$。为评价模型拟合效果，本文选取 MAE、RMSE、最大误差和决定系数 $R^2$ 作为评价指标。其中 MAE 反映平均绝对偏差，RMSE 对较大误差更敏感，最大误差用于衡量最坏情况下的偏差，$R^2$ 用于衡量模型对数据波动的解释程度。若模型用于预测，则进一步划分训练集和测试集，以测试集误差作为模型泛化能力的主要依据，并结合残差图判断模型是否存在系统偏差。
\end{stepbox}

\begin{stepbox}{结论段落模板}
...
\begin{enumerate}[label={[\arabic*]}]
  ...
  \item scikit-learn Documentation, \emph{Metrics and scoring: quantifying the quality of predictions}. \url{https://scikit-learn.org/stable/modules/model_evaluation.html}
  \item scikit-learn Documentation, \emph{Regression metrics: mean\_absolute\_error, mean\_squared\_error, root\_mean\_squared\_error, r2\_score}. \url{https://scikit-learn.org/stable/api/sklearn.metrics.html}
